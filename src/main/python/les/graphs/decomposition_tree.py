...
class Edge(dict):
    ...
    def __init__(self, source, dest, shared_variables=[]):
        ...
        shared_variables = map(str, shared_variables)
...
class DecompositionTree(nx.DiGraph):
    ...
    def __str__(self):
        return f'{self.__class__.__name__}[num_nodes={self.get_num_nodes()}, num_edges={self.get_num_edges()}]'
    ...
    def add_node(self, node_or_model):
        ...
        super().add_node(model.get_name(), node=node)
        return node
    ...
    def get_nodes(self):
        return [j['node'] for i, j in self.nodes(data=True)]

    def get_node(self, item):
        return self.nodes[item]['node']
    ...
    def remove_node(self, node):
        for other in self.predecessors(node):
            self.get_node(node)['model'].remove_dependecy(self.get_node(other)['model'])
        for other in self.successors(node):
            self.get_node(other)['model'].remove_dependecy(self.get_node(node)['model'])
        super(DecompositionTree, self).remove_node(node)

    def get_edge_between(self, node1, node2):
        return Edge(**self.get_edge_data(node1, node2)['data'])
    ...
    def add_edge(self, *args, **kwargs):
        ...
        super().add_edge(edge.get_source().get_name(),
        ...
        for node_name in list([edge.get_source().get_name(),
                               edge.get_dest().get_name()]):
    ...
    def get_models(self):
        ...
        '''
        return [n['model'] for n in self.get_nodes()]
