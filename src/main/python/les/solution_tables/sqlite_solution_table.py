...
class SQLiteSolutionTable(db_solution_table_base.DBSolutionTableBase):
    ...
    def set_decomposition_tree(self, tree):
        ...
        for model in tree.get_models():
            node = tree.get_node(model.get_name())
    ...
    def write_solution(self, model, solution):
        ...
        for name in self._decomposition_tree.neighbors(model.get_name()):
            ...
            stmt += [' AND '.join(
                ['%s.%s=%d' % (name, i, solution.get_variable_value_by_name(i)) for i in edge.get_shared_variables()])]
        dep_true_vars = ''
        if stmt:
        ...
        indices = range(len(solution.get_variables_names()))
        if not indices:
            return
        node = self._decomposition_tree.get_node(model.get_name())
